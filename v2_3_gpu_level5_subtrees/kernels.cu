...
__global__ void __launch_bounds__(1024, 1) kernelPartialSolve(const ulonglong1* __restrict__ occupiedfromid, int maxpartials, int* __restrict__ partials, ushort2* __restrict__ partialpieces, ulonglong1* __restrict__ partialoccupied, ushort4* __restrict__ partialids)
{
                                ...
                                while (1)
                                {
                                    ...
                                    if (!(occupied2 & occupied3))
                                    {
                                        ...
                                        for (int piece4 = 0; piece4 < 13; piece4++)
                                        {
                                            ...
                                            {
                                                pieces4 |= pieces3;
                                                int id4 = (target4 << 9) + (piece4 << 5);
                                                while (1)
                                                {
                                                    uint64_t occupied4 = occupiedfromid[id4].x;
                                                    if (!occupied4)
                                                    {
                                                        break;
                                                    }
                                                    if (!(occupied3 & occupied4))
                                                    {
                                                        ...
                                                        if (slot < maxpartials)
                                                        {
                                                            partialpieces[slot] = ushort2({ static_cast<uint16_t>(0x1fff ^ pieces4), static_cast<uint16_t>(id4) });
...
extern "C" void cudaPartialSolve(const ulonglong1* occupiedfromid, int maxpartials, int* partials, ushort2* partialpieces, ulonglong1* partialoccupied, ushort4* partialids)
{
    ...
    while (1)
    {
        if (!pieces)
        {
            ...
            if (level)
            {
                ...
                id3 = sharedids3[slot];
                id4 = sharedids4[slot];
                ids[0] = sharedids5[slot];
                ids[1] = sharedids6[slot];
                ids[2] = sharedids7[slot];
                __threadfence_block(); // release
        ...
        if (occupiedn)
        {
            ...
            if (level + 1 == 3)
            {
                ...
                if (atomicAnd(&sharedfree, ~slotbit) & slotbit)
                {
                    ...
                    sharedids4[threadIdx.x] = id4;
                    sharedids5[threadIdx.x] = ids[0];
                    sharedids6[threadIdx.x] = ids[1];
                    sharedids7[threadIdx.x] = ids[2];
                    __threadfence_block(); // release
            ...
            if (level + 1 == 8)
            {
                ...
                if (slot < maxsolutions)
                {
                    ...
                    solutionids[slot][1] = ushort4({ id4, ids[0], ids[1], ids[2] });
                    solutionids[slot][2] = ushort4({ ids[3], ids[4], ids[5], ids[6] });
                    solutionids[slot][3] = ushort4({ ids[7], static_cast<uint16_t>(0), static_cast<uint16_t>(0), static_cast<uint16_t>(0) });
                }
                id++;
                continue;
            }
...
extern "C" void cudaSolve(const ulonglong1* occupiedfromid, const int* partials, const ushort2* partialpieces, const ulonglong1* partialoccupied, const ushort4* partialids, int* active, int* producers, int* partial, int maxsolutions, int* solutions, ushort4(*solutionids)[4], int multiprocessors)
{
