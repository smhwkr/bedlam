...
int main()
{
    ...
    ushort1 uidfrompiece[13];
    ulonglong1 occupiedfromuid[5203] = {};
    int uid = 1;
    for (int piece = 0; piece < 13; piece++)
    {
        uidfrompiece[piece].x = static_cast<uint16_t>(uid);
        for (auto& occupied : occupiedset[piece])
        {
            occupiedfromuid[uid].x = occupied;
            uid++;
        }
    }

    ushort1 uidfromcid[64 * 325] = {};
    ...
    for (int target = 0; target < 64; target++)
    {
        for (int piece = 0; piece < 13; piece++)
        {
            ...
            int uid = uidfrompiece[piece].x;
            for (auto& occupied : occupiedset[piece])
            {
                if (!(occupied & unsetmask) && (occupied & setmask))
                {
                    uidfromcid[cid].x = static_cast<uint16_t>(uid);
    ...
    ulonglong1* dev_occupiedfromuid = nullptr;
    cudaMalloc(&dev_occupiedfromuid, 5203 * sizeof(ulonglong1));

    cudaMemcpy(dev_occupiedfromuid, occupiedfromuid, 5203 * sizeof(ulonglong1), cudaMemcpyHostToDevice);

    ushort1* dev_uidfromcid = nullptr;
    cudaMalloc(&dev_uidfromcid, 64 * 325 * sizeof(ushort1));

    cudaMemcpy(dev_uidfromcid, uidfromcid, 64 * 325 * sizeof(ushort1), cudaMemcpyHostToDevice);
    ...
    cudaSolve(dev_occupiedfromuid, dev_uidfromcid, dev_partials, dev_partialpieces, dev_partialoccupied, dev_partialcids, host_active, host_producers, dev_partial, maxsolutions, dev_solutions, dev_solutioncids, multiprocessors);
    ...
    cudaFree(dev_uidfromcid);

    cudaFree(dev_occupiedfromuid);
